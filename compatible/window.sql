...
SELECT avg(k) OVER w FROM kv WINDOW w AS ();
...
INSERT INTO kv VALUES
(9, 2, 9, .1, DEFAULT, DEFAULT, DEFAULT, DEFAULT),
(10, 4, 9, .2, DEFAULT, DEFAULT, DEFAULT, DEFAULT),
(11, NULL, 9, .3, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
...
INSERT INTO kv VALUES (12, -1, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
...
SELECT avg(price) OVER (ORDER BY price ROWS 1 PRECEDING) FROM products;

-- Test 170: statement (line 2063)
-- query error: invalid window frame
\set ON_ERROR_STOP 0
...
-- query error: invalid FILTER usage
\set ON_ERROR_STOP 0
...
SELECT (SELECT count(DISTINCT group_name) FROM products), (SELECT count(DISTINCT product_name) FROM products) FROM products;

-- Test 212: statement (line 2448)
-- query error: invalid RANGE frame
\set ON_ERROR_STOP 0
...
-- query error: invalid GROUPS frame
\set ON_ERROR_STOP 0
...
-- query error: window functions are not allowed in window definitions
\set ON_ERROR_STOP 0
...
SELECT
    min(a) OVER (PARTITION BY 1) AS min,
    max(a) OVER (PARTITION BY 1) AS max
FROM
    (SELECT 1 AS a);
...
-- query error: GROUPS mode requires an ORDER BY clause
\set ON_ERROR_STOP 0
...
-- query error: invalid string_agg signature
\set ON_ERROR_STOP 0
...
CREATE TABLE t54604 (s TEXT);
INSERT INTO t54604 VALUES ('a'), ('b'), ('c');
...
SELECT x,
       (var_pop(x) OVER (ORDER BY x) * count(x) OVER (ORDER BY x)) as sqrdiff,
...
INSERT INTO t VALUES ('294276-01-01'::DATE), ('1999-01-08'::DATE);
-- SET vectorize=off;
