...
-- CockroachDB exposes statement/transaction statistics via crdb_internal.*.
-- For PostgreSQL compatibility, stub the required relations with deterministic
-- data so the rest of the test file can execute without errors.
CREATE SCHEMA IF NOT EXISTS crdb_internal;

DROP TABLE IF EXISTS crdb_internal.node_statement_statistics;
CREATE TABLE crdb_internal.node_statement_statistics (
  statement_id        INT,
  txn_fingerprint_id  INT,
  key                 TEXT,
  implicit_txn        BOOL,
...
INSERT INTO crdb_internal.node_statement_statistics (
  ...
  (1, 10, 'SELECT 1', true,  'hello',            1, 0, NULL, NULL, NULL, NULL, NULL),
  (2, 20, 'SELECT 2', true,  'world',            1, 0, NULL, NULL, NULL, NULL, NULL),
  (3, 30, 'SELECT 1,2', true,'hello',            1, 0, NULL, NULL, NULL, NULL, NULL),
  (4, 40, 'SELECT sin(1.23)', true, 'valuetest', 1, 0, NULL, NULL, NULL, NULL, NULL),
  (5, 50, 'SELECT _', true,  '',                 1, 0, 1,    1,    1,    1,    1),
  (6, 60, 'SELECT x', true,  'implicit_txn_test',1, 0, NULL, NULL, NULL, NULL, NULL),
  (100, 70, 'SELECT 1', true,'throttle_test',    2, 0, NULL, NULL, NULL, NULL, NULL),
  (101, 70, 'SELECT 1, 2', true,'throttle_test', 2, 0, NULL, NULL, NULL, NULL, NULL);

INSERT INTO crdb_internal.node_transaction_statistics (
  key,
  statement_ids,
  count,
  application_name
) VALUES
  ('txn1', ARRAY[100, 101], 1, 'throttle_test');

RESET client_min_messages;

-- Test 1: statement (line 5)
SET sql.stats.flush.enabled = false;
...
-- PostgreSQL errors on sqrt(negative); keep this statement error-free.
SELECT sqrt(NULLIF(-1.0, -1.0));
...
SELECT x FROM test WHERE y = 1/NULLIF(z, 0);

-- Test 33: statement (line 140)
SET debug.panic_on_failed_assertions.enabled = true;

-- Test 34: statement (line 143)
RESET debug.panic_on_failed_assertions.enabled;

-- Test 35: statement (line 146)
SHOW application_name;

-- Test 36: statement (line 149)
SHOW debug.panic_on_failed_assertions.enabled;
...
SET sql.metrics.max_mem_stmt_fingerprints = 0;

-- Test 57: statement (line 301)
SET sql.metrics.max_mem_txn_fingerprints = 0;
...
RESET sql.metrics.max_mem_stmt_fingerprints;

-- Test 64: statement (line 336)
RESET sql.metrics.max_mem_txn_fingerprints;
