...
-- CockroachDB column families/FAMILY and STORING clauses are not supported in
-- PostgreSQL. Map STORING to INCLUDE and drop FAMILY clauses; remove @index
-- hints and translate UPSERT.
...
SELECT y, z, w FROM t WHERE y = 2 ORDER BY y;

-- Test 3: statement (line 20)
DROP TABLE IF EXISTS t;

-- Setup for tests 4-7: exercise INCLUDE columns with an ON CONFLICT update.
CREATE TABLE t (
  x INT,
  y INT PRIMARY KEY,
  z TEXT,
...
CREATE INDEX i ON t (y) INCLUDE (z, v);
INSERT INTO t VALUES (1, 2, '1', 1);

-- Test 4: query (line 34)
SELECT y, z, v FROM t ORDER BY y;

-- Test 5: statement (line 40)
INSERT INTO t VALUES (10, 2, '10', 10)
...
-- Setup for tests 8-10: UPSERT -> INSERT ... ON CONFLICT.
CREATE TABLE t (
  x INT PRIMARY KEY,
  y TEXT,
  z NUMERIC,
...
INSERT INTO t VALUES (9, '10', 11.000, 12), (1, '3', 5.0, 16)
ON CONFLICT (x) DO UPDATE
  SET y = EXCLUDED.y, z = EXCLUDED.z, w = EXCLUDED.w;

-- Test 9: query (line 67)
SELECT y, z, w FROM t ORDER BY x;
...
CREATE INDEX i ON t (y) INCLUDE (z, w, v);

-- Test 14: query (line 93)
SELECT y, z, w, v FROM t ORDER BY y;
...
ALTER TABLE t ADD COLUMN u INT DEFAULT 20;

-- Test 18: statement (line 113)
CREATE INDEX i ON t (y) INCLUDE (z, w, v, u);

-- Test 19: query (line 116)
SELECT y, z, w, v, u FROM t ORDER BY y;
...
CREATE UNIQUE INDEX t42992_y_idx ON t42992 (y) INCLUDE (z);

-- Test 21: statement (line 127)
INSERT INTO t42992 VALUES (now(), NULL, 2);

-- Test 22: query (line 130)
SELECT y, z FROM t42992 ORDER BY y NULLS FIRST;
