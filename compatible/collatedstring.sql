...
-- Define a small set of ICU collations that match the names used in the
-- CockroachDB-derived test cases.
CREATE COLLATION IF NOT EXISTS public.en (provider = icu, locale = 'en');
CREATE COLLATION IF NOT EXISTS public.de (provider = icu, locale = 'de');
CREATE COLLATION IF NOT EXISTS public.da (provider = icu, locale = 'da-u-kf-lower');

-- "ks" (collation strength) controls case sensitivity. In CockroachDB,
-- en_u_ks_level1 is case-insensitive; in PostgreSQL this requires a
-- non-deterministic ICU collation.
CREATE COLLATION IF NOT EXISTS public.en_u_ks_level1
  (provider = icu, locale = 'en-u-ks-level1', deterministic = false);
CREATE COLLATION IF NOT EXISTS public.de_u_ks_level1
  (provider = icu, locale = 'de-u-ks-level1', deterministic = false);
CREATE COLLATION IF NOT EXISTS public.en_u_ks_level2
  (provider = icu, locale = 'en-u-ks-level2', deterministic = false);

-- Additional collation names referenced in the tests.
CREATE COLLATION IF NOT EXISTS public."en_US" (provider = icu, locale = 'en-US');
CREATE COLLATION IF NOT EXISTS public."en_US_u_ks_level2"
  (provider = icu, locale = 'en-u-ks-level2', deterministic = false);
CREATE COLLATION IF NOT EXISTS public."en-US-u-ks-level2"
  (provider = icu, locale = 'en-u-ks-level2', deterministic = false);
CREATE COLLATION IF NOT EXISTS public."en-us-u-ks-level2"
  (provider = icu, locale = 'en-u-ks-level2', deterministic = false);
...
-- Postgres requires both operands in a comparison to have the same collation.
SELECT ('A' COLLATE en) = (('a' COLLATE de) COLLATE en);
...
CREATE TABLE foo (a TEXT COLLATE en_u_ks_level2);
PREPARE x (text) AS INSERT INTO foo VALUES ($1 COLLATE en_u_ks_level2) RETURNING a;
...
CREATE TABLE quoted_coll (a TEXT COLLATE "en-US-u-ks-level2");
SELECT column_name, collation_name
FROM information_schema.columns
WHERE table_schema = 'public'
  AND table_name = 'quoted_coll'
ORDER BY ordinal_position;
...
CREATE TABLE disallowed_c(a text COLLATE "C");

-- Test 74: statement (line 499)
CREATE TABLE disallowed_posix(a text COLLATE "POSIX");

-- Test 75: statement (line 508)
CREATE TABLE nocase_strings (s TEXT COLLATE "en-US-u-ks-level2");
CREATE TABLE nocase_strings2 (id INT PRIMARY KEY, s TEXT COLLATE "en-US-u-ks-level2");
CREATE TABLE collation_name_case (s TEXT COLLATE "en-US-u-ks-level2");
...
FROM information_schema.columns
WHERE table_schema = 'public'
  AND table_name = 'collation_name_case'
ORDER BY ordinal_position;
...
-- Keep it commented to avoid breaking the rest of the script under psql.
-- SELECT s FROM nocase_strings WHERE s = ('bbb' COLLATE "en-us-u-ks-l"evel2");
...
CREATE TABLE t65631(a CHAR(3), b CHAR(3) COLLATE en);
...
CREATE TABLE test_collate (
  id INT PRIMARY KEY,
  string_field TEXT COLLATE en_u_ks_level1
);
...
CREATE TABLE str_arr (a TEXT[], b TEXT);
