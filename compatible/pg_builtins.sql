...
-- Test 2: query (line 6)
-- COMMENTED: PostgreSQL aclexplode takes aclitem[] not text[]
-- SELECT aclexplode(ARRAY[]::text[]);

-- Test 3: query (line 10)
-- COMMENTED: PostgreSQL aclexplode expects aclitem[], not text[]
-- SELECT aclexplode(ARRAY['foo']);

-- Test 4: statement (line 15)
-- COMMENTED: 'root' role doesn't exist in PostgreSQL
...
DROP TABLE IF EXISTS is_visible CASCADE;
...
-- COMMENTED: Logic test directive: let $table_in_db2_id
SELECT c.oid FROM pg_class c WHERE c.relname = 'table_in_db2';

-- COMMENTED: Logic test directive: let $type_in_db2_id
SELECT t.oid FROM pg_type t WHERE t.typname = 'type_in_db2';

-- COMMENTED: Logic test directive: let $func_in_db2_id
...
DROP TABLE IF EXISTS is_updatable CASCADE;
...
-- COMMENTED: Logic test directive: query error configuration setting.*not supported
...
DROP TABLE IF EXISTS numeric CASCADE;
...
-- COMMENTED: CockroachDB-specific: SELECT to_regnamespace('crdb_internal');
...
DROP TABLE IF EXISTS pg_indexes CASCADE;
...
DROP TABLE IF EXISTS t144384 CASCADE;
