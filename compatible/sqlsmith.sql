-- PostgreSQL compatible tests from sqlsmith
-- 15 tests

-- Test 1: statement (line 5)
-- (skipped on PostgreSQL) references CockroachDB-only objects/types (crdb_internal.*, BYTES, xor_agg).
...
-- (skipped on PostgreSQL) references CockroachDB-only objects (crdb_internal.cluster_queries).
...
CREATE TABLE table5 (col0 TIME NULL, col1 OID, col3 INET, PRIMARY KEY (col1));

-- Test 6: statement (line 25)
INSERT INTO table8 (col1, col2, col4, col6)
VALUES ('19:06:18.321589'::TIME, NULL::BYTEA, NULL::OID, NULL::NAME)
UNION ALL (SELECT NULL::TIME, NULL::BYTEA, NULL::OID, NULL::NAME FROM table5 AS tab_8);
...
SELECT
    with_20394.col_162526::UUID AS col_162534,
...
-- Test 11: statement (line 145)
CREATE TABLE t (d) AS VALUES ('2001-01-01'::DATE);

-- Test 12: query (line 148)
SELECT * FROM t WHERE (d = d) OR (d = d);

-- Test 13: query (line 154)
SELECT (ARRAY[1])[NULL];

-- Test 14: statement (line 160)
CREATE TABLE ab (
  a INT,
