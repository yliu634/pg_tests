...
CREATE TABLE s (x BYTEA);

-- Test 14: query (line 57)
SELECT * FROM s WHERE x > '\x00'::bytea;

-- Test 15: statement (line 61)
INSERT INTO s(x) VALUES
  (convert_to('qwe', 'UTF8')),
  (convert_to('start', 'UTF8') || convert_to('end', 'UTF8'));

-- Test 16: statement (line 64)
INSERT INTO s(x) VALUES ('\xfffefd'::bytea);

-- Test 17: query (line 67)
SELECT length(x), encode(x, 'escape') FROM s;
...
SELECT VARCHAR(4) 'foo', CHAR(2) 'bar', VARCHAR(1) 'cat';
...
\set ON_ERROR_STOP 0
CREATE TABLE t15050b (c DECIMAL DEFAULT CASE WHEN now() < 'Not Timestamp' THEN 2 ELSE 2 END);
\set ON_ERROR_STOP 1
...
SELECT ''::bytea BETWEEN ''::bytea AND ''::bytea;
...
SELECT CASE WHEN true THEN 1234::OID ELSE COALESCE(NULL::OID, NULL::OID) END;
...
SELECT ARRAY[]::TIMESTAMPTZ[] >
       SOME (VALUES
         (ARRAY[TIMESTAMPTZ '1969-12-29T21:20:13+01']),
...
CREATE DOMAIN typ83496 AS TEXT;
CREATE TABLE t83496 (c typ83496);
INSERT INTO t83496 VALUES ('bar');
