...
CREATE TABLE x_arr (y JSONB[]);

-- Test 19: statement (line 92)
CREATE TABLE foo (pk INT GENERATED BY DEFAULT AS IDENTITY, bar JSONB);

-- Test 20: statement (line 95)
CREATE VIEW x_view AS SELECT array_agg(bar) FROM foo;
...
CREATE INDEX x_j_gin ON x USING GIN (j);
...
SELECT '{"a": {"b": ["foo"]}}'::JSONB #- ARRAY['a', 'b', '0'];

-- Test 140: statement (line 766)
CREATE TABLE json_family (a INT PRIMARY KEY, b JSONB);
...
CREATE TABLE t49143 (k INT PRIMARY KEY, j JSONB);
...
SELECT j FROM t49143 WHERE NOT (j->0 = '2'::JSONB) ORDER BY k;

-- Test 147: query (line 811)
SELECT j FROM t49143 WHERE NOT (j->'b' = '1'::JSONB) ORDER BY k;

-- Test 148: query (line 818)
SELECT j FROM t49143 WHERE NOT (j -> 0 @> '[1]'::JSONB) ORDER BY k;

-- Test 149: query (line 825)
SELECT j FROM t49143 WHERE NOT (j -> 'b' @> '[1]'::JSONB) ORDER BY k;

-- Setup for tests using jsonb key deletion with a dynamic key column.
CREATE TABLE t57165 (rowid INT PRIMARY KEY, j JSONB, s TEXT);
INSERT INTO t57165 VALUES
  (1, '{"foo": 1, "bar": 2}'::JSONB, 'foo'),
  (2, '{"foo": 1, "bar": 2}'::JSONB, 'bar');
...
SELECT pg_input_is_valid('{"hello": {}}', 'json');

-- Test 156: query (line 872)
SELECT pg_input_is_valid('{"foo": {"bar": 1, "one": 1, "two": 2', 'json');

-- Test 157: query (line 877)
SELECT pg_input_is_valid('[{"bar": 1}, {"bar": 2}]', 'json');

-- Test 158: query (line 882)
SELECT pg_input_is_valid(NULL, 'json');

-- Test 159: statement (line 890)
CREATE TABLE t81647(j JSONB);
...
SELECT j::TEXT
FROM (VALUES ('1.0'::JSONB), ('1'::JSONB), ('1.00'::JSONB)) v(j)
ORDER BY 1;
