...
-- Test 9b: statement (added for PostgreSQL)
CREATE FUNCTION f_exprstar() RETURNS INT AS
$$
...
FROM pg_catalog.pg_proc WHERE proname LIKE 'f\_%' ORDER BY proname;

-- Test 15: query (line 107)
SELECT pg_get_functiondef('f_subquery'::regproc::oid);

-- Test 16: query (line 121)
SELECT pg_get_functiondef('f_allcolsel_alias'::regproc::oid);
...
-- Ensure DROP COLUMN without CASCADE errors in PostgreSQL (functions don't block
-- DROP COLUMN the way CRDB's dependency graph does).
CREATE VIEW v_twocol_b AS SELECT b FROM t_twocol_prime;

-- Test 35: statement (line 216)
\set ON_ERROR_STOP 0
ALTER TABLE t_twocol_prime DROP COLUMN b;
\set ON_ERROR_STOP 1
...
ALTER TABLE t_twocol_prime DROP COLUMN b CASCADE;
