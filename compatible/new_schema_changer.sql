...
DROP TABLE IF EXISTS blog_posts2 CASCADE;
...
CREATE VIEW v7_dep AS (SELECT i FROM t6 WHERE k < 'a'::typ6);
...
DROP TABLE IF EXISTS defaultdb CASCADE;
...
SELECT name, state;
-- COMMENTED: CockroachDB-specific:      FROM crdb_internal.tables
...
DROP TABLE IF EXISTS db1 CASCADE;
...
-- COMMENTED: Logic test directive: let $db1_id
...
-- COMMENTED: CockroachDB-specific FAMILY: CREATE TABLE trewrite(k INT PRIMARY KEY, ts TIMESTAMPTZ, FAMILY (k,ts));
...
DROP TABLE IF EXISTS tIndex CASCADE;
CREATE TABLE tIndex (;
  a INT PRIMARY KEY,
  b INT,
...
CREATE INDEX bar4 ON tIndex USING gin((ARRAY[a,b]););

-- Test 214: statement (line 894)
DROP TABLE IF EXISTS tIndx2 CASCADE;
CREATE TABLE tIndx2 (a INT PRIMARY KEY, b INT, INDEX ((a+b)));

-- Test 215: statement (line 897)
DROP TABLE IF EXISTS tIndx3 CASCADE;
...
DROP TABLE IF EXISTS tIndx CASCADE;
CREATE TABLE tIndx (;
  a INT PRIMARY KEY,
  b INT,
  c INT
...
DROP TABLE IF EXISTS create_idx_drop_column CASCADE;
...
DROP TABLE IF EXISTS t1dr CASCADE;
...
DROP TABLE IF EXISTS t1ev CASCADE;
CREATE TABLE t1ev(name varchar(256));

-- Test 254: statement (line 1097)
DROP TABLE IF EXISTS t2ev CASCADE;
...
DROP TABLE IF EXISTS fooev CASCADE;
...
DROP TABLE IF EXISTS multi_t1 CASCADE;
...
DROP TABLE IF EXISTS sbt1 CASCADE;
...
DROP TABLE IF EXISTS seqowner CASCADE;
...
CREATE VIEW child AS SELECT count(*) FROM parent;
...
EXPLAIN (DDL) DROP INDEX parent CASCADE;
...
DROP TABLE IF EXISTS stmt_ctrl CASCADE;
...
DROP TABLE IF EXISTS stmt_ctrl_sub CASCADE;
...
DROP TABLE IF EXISTS multiple_pk_attempt CASCADE;
CREATE TABLE multiple_pk_attempt(n INT PRIMARY KEY, j INT);

-- COMMENTED: Logic test directive: let $use_decl_sc
