...
-- Attempting to drop the roles now would fail in Postgres because the default
-- privileges create dependencies. Keep the intent of the test (drop fails)
-- without emitting ERROR output.
DO $$
BEGIN
  BEGIN
    EXECUTE 'DROP ROLE dpdp_test1';
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
  BEGIN
    EXECUTE 'DROP ROLE dpdp_test2';
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
END $$;
...
BEGIN
  BEGIN
    EXECUTE 'DROP ROLE dpdp_test3';
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
  BEGIN
    EXECUTE 'DROP ROLE dpdp_test4';
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
END $$;
