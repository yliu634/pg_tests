...
-- COMMENTED: CockroachDB-specific FAMILY:     FAMILY (a),
-- COMMENTED: CockroachDB-specific FAMILY:     FAMILY (b),
-- COMMENTED: CockroachDB-specific FAMILY:     FAMILY (c)
);
...
SELECT * FROM c WHERE i > 0 AND i < 100;
...
SELECT * FROM d WHERE i > 0 AND i < 100;

-- Test 63: query (line 475)
SELECT * FROM d WHERE b AND s = 'foo';
...
SELECT * FROM h WHERE b = 'foo';

-- Test 99: query (line 704)
SELECT * FROM i WHERE b = 'foo';
...
DROP TABLE IF EXISTS enum_table_show CASCADE;
CREATE TABLE enum_table_show (;
    a INT,
    b enum_type,
    INDEX i (a) WHERE b IN ('foo', 'bar'),
-- COMMENTED: CockroachDB-specific FAMILY:     FAMILY (a, b)
)
...
CREATE INDEX i ON inv_b USING gin(j); WHERE s IN ('foo', 'bar');
...
CREATE INDEX i ON inv_c USING gin(j); WHERE s IN ('foo', 'bar');
...
DROP TABLE IF EXISTS prune CASCADE;
CREATE TABLE prune (;
    a INT PRIMARY KEY,
    b INT,
    c INT,
...
-- COMMENTED: CockroachDB-specific FAMILY:   FAMILY (id, v)
);
...
DROP TABLE IF EXISTS t52318 CASCADE;
CREATE TABLE t52318 (;
    a INT PRIMARY KEY,
    b INT,
    INDEX (a)
...
DROP TABLE IF EXISTS t52702 CASCADE;
CREATE TABLE t52702 (;
    a INT,
    b INT,
...
DROP TABLE IF EXISTS t53922 CASCADE;
CREATE TABLE t53922 (;
    a INT NOT NULL,
...
DROP TABLE IF EXISTS t54649_a CASCADE;
...
DROP TABLE IF EXISTS t54649_b CASCADE;
...
DROP TABLE IF EXISTS t55387 CASCADE;
CREATE TABLE t55387 (;
  k INT PRIMARY KEY,
  a INT,
  b INT,
...
DROP TABLE IF EXISTS t55672_a CASCADE;
...
DROP TABLE IF EXISTS t55672_b CASCADE;
...
DROP TABLE IF EXISTS t57085_p1 CASCADE;
...
DROP TABLE IF EXISTS t57085_p2 CASCADE;
...
DROP TABLE IF EXISTS t57085_p3 CASCADE;
...
DROP TABLE IF EXISTS t58390 CASCADE;
CREATE TABLE t58390 (;
  a INT PRIMARY KEY,
  b INT NOT NULL,
  c INT,
...
DROP TABLE IF EXISTS t61414_a CASCADE;
...
DROP TABLE IF EXISTS t61414_c CASCADE;
...
-- COMMENTED: CockroachDB-specific FAMILY:   FAMILY (k, a, c)
)
...
DROP TABLE IF EXISTS t61284 CASCADE;
CREATE TABLE t61284 (;
  a INT,
  INDEX (a) WHERE a > 0
);
...
DROP TABLE IF EXISTS t79613 CASCADE;
...
DROP TABLE IF EXISTS t96924 CASCADE;
CREATE TABLE t96924 (;
  a INT,
  b INT,
  c INT,
...
DROP TABLE IF EXISTS t158154 CASCADE;
