...
-- CockroachDB `SHOW CONSTRAINTS` equivalent for PostgreSQL.
CREATE OR REPLACE FUNCTION pg_show_constraints(target text)
RETURNS TABLE(
...
LANGUAGE SQL STABLE AS $$
  ...
  JOIN parts p ON p.schemaname = n.nspname AND p.tablename = r.relname
...
SELECT * FROM pg_show_constraints('test.child') ORDER BY constraint_name;
...
ALTER TABLE stock ADD COLUMN c INT GENERATED ALWAYS AS (quantity + 4) STORED, ADD COLUMN d INT DEFAULT 23, ADD CONSTRAINT bar UNIQUE (c);
...
ALTER TABLE shopping ADD COLUMN c int GENERATED ALWAYS AS (quantity::int / 0) STORED;
...
-- statement ok
ALTER TABLE cutlery ADD COLUMN c INT GENERATED ALWAYS AS (quantity + 4) STORED, ADD COLUMN d INT;

-- query TIII rowsort
SELECT * FROM test.cutlery ORDER BY 1,2,3,4;
...
ALTER TABLE customers ADD COLUMN d INT DEFAULT 15, ADD COLUMN e INT GENERATED ALWAYS AS (d + (i-1)) STORED;
...
DROP INDEX orders2_product_idx;
...
SELECT * FROM pg_show_constraints('check_table') ORDER BY constraint_name;
...
ALTER TABLE t ADD COLUMN d INT GENERATED ALWAYS AS (a - 1) STORED;
...
ALTER TABLE t ADD COLUMN b INT GENERATED ALWAYS AS (a) STORED;
...
SELECT * FROM pg_show_constraints('y') ORDER BY constraint_name;
...
-- query II rowsort
SELECT * FROM self_ref_fk ORDER BY 1,2;
...
SELECT * FROM pg_show_constraints('t_52501_valid') ORDER BY constraint_name;
...
SELECT * FROM pg_show_constraints('child_54265') ORDER BY constraint_name;
