...
DROP TABLE IF EXISTS bits, obits, obitsa CASCADE;
CREATE TABLE bits (
  a VARBIT,
  b VARBIT,
  c VARBIT,
...
SELECT ARRAY[B'101011'] AS a, ARRAY[B'111001']::VARBIT[] AS b;
...
SELECT * FROM obitsa ORDER BY x;

-- Test 33: statement (line 330)
-- Validate bit-string content without raising a cast error.
SELECT ('AB' ~ '^[01]+$') AS is_valid;

-- Test 34: statement (line 334)
SELECT ('AB' ~ '^[01]+$') AS is_valid;

-- Test 35: statement (line 344)
SELECT ('xZZ' ~ '^[01]+$') AS is_valid;
