...
SELECT 'lower(text)'::REGPROCEDURE;
...
-- COMMENTED: Schema "foo" does not exist in this PG test database.
...
-- COMMENTED: The following CRDB logic-test error cases are not executed in this PG-compatible version.
...
SELECT 'postgres'::REGROLE, 'postgres'::REGROLE::OID;
...
SELECT (SELECT c.oid FROM pg_class c WHERE c.relname = 'a')::oid::regclass;

-- Test 28: query (line 217)
SELECT (SELECT t.oid FROM pg_type t WHERE t.typname = 'typ')::oid::regtype;
...
DROP TABLE IF EXISTS hasCase CASCADE;
DROP TYPE IF EXISTS typHasCase CASCADE;
...
DROP TABLE IF EXISTS "quotedCase" CASCADE;
CREATE TABLE "quotedCase" (id INT PRIMARY KEY);

-- Test 35: statement (line 251)
DROP TYPE IF EXISTS "typQuotedCase" CASCADE;
...
-- COMMENTED: CockroachDB-specific crdb_internal.* helpers.
...
DROP TABLE IF EXISTS "regression_53686""" CASCADE;
...
SELECT 'pg_catalog."radians"(double precision)'::regprocedure;
...
DROP TABLE IF EXISTS regression_62205 CASCADE;
...
DROP TABLE IF EXISTS table_with_indexes CASCADE;
...
DROP TABLE IF EXISTS other_schema.table_with_indexes CASCADE;
...
DROP TABLE IF EXISTS t118273 CASCADE;
...
DROP TABLE IF EXISTS t123474 CASCADE;
CREATE TABLE t123474 (
  col_0 REGROLE,
  col_1 OID,
  col_2 INT
);
CREATE INDEX t123474_col1_desc_idx ON t123474 (col_1 DESC) INCLUDE (col_2);
CREATE INDEX t123474_col0_idx ON t123474 (col_0) INCLUDE (col_1);
...
SELECT t2.col_1
  FROM t123474 AS t1 JOIN t123474 AS t2 ON (t1.col_0::oid) = (t2.col_1)
  ORDER BY t1.col_0;
...
DROP TABLE IF EXISTS t123548_1 CASCADE;
DROP TABLE IF EXISTS t123548_2 CASCADE;
CREATE TABLE t123548_1 (col1 REGCLASS);
CREATE INDEX t123548_1_col1_idx ON t123548_1 (col1);
CREATE TABLE t123548_2 (col2 OID PRIMARY KEY);
...
SELECT col1 FROM t123548_1 JOIN t123548_2 ON col1::oid = col2;
