...
SELECT 'a'::regclass::oid::regclass;

-- Test 28: query (line 217)
SELECT 'typ'::regtype::oid::regtype;
...
-- CockroachDB-only privilege target (database switching is emulated via schemas in PG tests).
-- GRANT ALL ON DATABASE test TO testuser;

-- Test 39: statement (line 276)
-- CockroachDB-only syntax.
...
-- CockroachDB-only optimizer test knobs.
