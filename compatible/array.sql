...
CREATE COLLATION IF NOT EXISTS public.en (provider = icu, locale = 'en');
CREATE COLLATION IF NOT EXISTS public.fr (provider = icu, locale = 'fr');
...
SELECT ARRAY['a', true::text, 1::text];

-- query T
SELECT ARRAY['a,', 'b{', 'c}', 'd', 'e f'];

-- Test 7: query (line 45)
SELECT ARRAY['1}'::BYTEA];
...
-- Expected ERROR (subquery must return only one column).
\set ON_ERROR_STOP 0
SELECT ARRAY(SELECT 1, 2);
\set ON_ERROR_STOP 1
...
-- Expected ERROR (cannot cast integer to UUID).
\set ON_ERROR_STOP 0
...
-- Expected ERROR (cannot subscript type record).
\set ON_ERROR_STOP 0
SELECT (1,2,3)[1];
\set ON_ERROR_STOP 1
...
CREATE TABLE boundedtable (b INT[10], c INT[10]);

-- Test 57: statement (line 447)
DROP TABLE boundedtable;

-- Test 58: statement (line 451)
CREATE TABLE badtable_array (b INT[][]);
...
-- Expected ERROR (CockroachDB SHOW CREATE TABLE is not supported in PostgreSQL).
\set ON_ERROR_STOP 0
SHOW CREATE TABLE a;
\set ON_ERROR_STOP 1
...
INSERT INTO a VALUES (ARRAY[]::INT[]);
...
-- PG adaptation: this section expects a string array table.
...
INSERT INTO a VALUES (ARRAY[]::BOOL[]), (ARRAY[TRUE]), (ARRAY[FALSE]), (ARRAY[TRUE, TRUE]), (ARRAY[FALSE, TRUE]);
...
CREATE TABLE a (b BYTEA[]);

-- Test 117: statement (line 758)
INSERT INTO a VALUES (ARRAY['foo'::BYTEA,'bar'::BYTEA,'baz'::BYTEA]);
...
SELECT array_append(ARRAY['foo' COLLATE en], 'bar' COLLATE en);
...
INSERT INTO t_bool VALUES (ARRAY[]::BOOL[]);
...
CREATE OR REPLACE FUNCTION jsonb_array_to_string_array(input JSONB)
RETURNS TEXT[]
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT CASE
           ...
           WHEN jsonb_typeof(input) <> 'array' THEN NULL
           ELSE COALESCE(
             ARRAY(SELECT jsonb_array_elements_text(input)),
             ARRAY[]::TEXT[]
           )
         END;
$$;
...
-- CRDB-only: missing setup for table `x` in this file.
-- UPDATE x SET a = ARRAY[]::INT[], b = ARRAY[]::INT[];
...
SELECT ARRAY['']::TEXT[] = ARRAY[]::TEXT[] FROM (VALUES (1)) WHERE ARRAY[B'']::BIT[] != ARRAY[]::BIT[];

-- Test 235: statement (line 1396)
CREATE TABLE array_single_family (a INT PRIMARY KEY, b INT[]);

-- Test 236: statement (line 1399)
INSERT INTO array_single_family VALUES(0,ARRAY[]::INT[]);
...
CREATE TABLE defvals (
    ...
    arr1 VARCHAR(100)[] NOT NULL DEFAULT ARRAY[]::VARCHAR(100)[],
    arr2 INT[] NOT NULL DEFAULT ARRAY[]::INT[]
);
...
CREATE TABLE defvals2 (
    id SERIAL NOT NULL PRIMARY KEY,
    arr1 VARCHAR(100)[] NOT NULL DEFAULT ARRAY[NULL]::VARCHAR(100)[],
    arr2 INT[] NOT NULL DEFAULT ARRAY[NULL]::INT[]
);
...
-- Expected ERROR (geography type requires PostGIS).
\set ON_ERROR_STOP 0
...
INSERT INTO t VALUES
  ...
  (2, ARRAY[NULL, NULL, NULL]::INT[], NULL),
  (NULL, ARRAY[NULL, NULL]::INT[], 3),
  (2, ARRAY[4, 5], 7);
...
-- PG adaptation: keep table `t` for the next query.
-- DROP TABLE t;
...
-- CRDB-only / mismatched setup: `t.x` is not an array in PostgreSQL here.
...
CREATE TABLE t (x BYTEA[]);

-- Test 300: statement (line 1811)
INSERT INTO t VALUES
  (ARRAY['\xff'::bytea, '\x00'::bytea]),
  (ARRAY[NULL, '\x01'::bytea, '\x01'::bytea, NULL]),
  (ARRAY[NULL, '\xff'::bytea]);
...
INSERT INTO t VALUES
  (ARRAY[1, 2], ARRAY[3, 4]),
  (ARRAY[NULL, NULL]::INT[], ARRAY[NULL, NULL]::INT[]),
  (ARRAY[]::INT[], ARRAY[]::INT[]),
  (ARRAY[]::INT[], ARRAY[NULL, 2]::INT[]),
...
SELECT CASE
  WHEN NULL THEN (('1'::BIGINT, ARRAY[]::INT2[]))
  ELSE ('2'::BIGINT, ARRAY[(-23791)::BIGINT])
END;
...
-- PG adaptation: setup table for array literal parsing tests.
CREATE TABLE t (
  a INT[],
  b FLOAT[],
  c UUID[],
...
SELECT array_remove(
  ARRAY[ROW(1,'cat')::kv, ROW(10,'fish')::kv, ROW(11,'zebra')::kv],
  ROW(10,'fish')::kv
);

-- Test 351: query (line 2183)
SELECT array_remove(
  ARRAY[ROW(1,'cat')::kv, ROW(10,'fish')::kv, ROW(11,'zebra')::kv, NULL::kv],
  NULL::kv
);

-- Test 352: query (line 2188)
SELECT array_remove(NULL::kv[], NULL::kv);
...
SELECT
  array_replace(
    ARRAY[ROW(1,'cat')::kv, NULL::kv, ROW(11,'zebra')::kv],
    NULL::kv,
    ROW(2,'dog')::kv
  ),
  array_replace(NULL::kv[], ROW(10,'fish')::kv, ROW(2,'dog')::kv);
...
SELECT array_position(NULL::kv[], ROW(33,'hippo')::kv);

-- Test 358: query (line 2222)
SELECT array_positions(
  ARRAY[ROW(1,'cat')::kv, ROW(11,'zebra')::kv, ROW(10,'fish')::kv, ROW(11,'zebra')::kv],
  ROW(11,'zebra')::kv
);
...
SELECT array_positions(NULL::kv[], ROW(33,'hippo')::kv);

-- Test 361: query (line 2242)
-- Expected ERROR (cannot compare dissimilar record column types).
\set ON_ERROR_STOP 0
...
-- query T
SELECT array_replace(
  array_append(ARRAY[ROW(1,'cat')::kv], ROW(2,true::text)::kv),
  ROW(1,'fish')::kv,
  ROW(3,'dog')::kv
);
...
SELECT array_replace(
  ARRAY[ROW(1,'cat')::kv, ROW(2,'dog')::kv],
  ROW(2,'dog')::kv,
  ROW(3,true::text)::kv
);
...
INSERT INTO t2_87919 (c) VALUES (ARRAY['03:23:06.042923'::TIME]);
