...
SELECT * FROM t1 INNER JOIN t2 ON t1.k = t2.x ORDER BY 1;
...
SELECT t2.y, t1.v FROM t1 INNER JOIN t2 ON t1.k = t2.x ORDER BY 1 DESC;
...
CREATE TABLE t41407 AS
	SELECT
		...
		g AS _bytes
		FROM
			generate_series(NULL::INT, NULL::INT) AS g;
