...
DO $$
BEGIN
  -- Included columns cannot duplicate key columns.
  EXECUTE 'CREATE INDEX error ON t (d) INCLUDE (d)';
EXCEPTION
  WHEN others THEN
    NULL;
END
...
DO $$
BEGIN
  -- Depending on what happened above, this may succeed or fail; either way, continue.
  EXECUTE 'CREATE INDEX error ON t (d) INCLUDE (a)';
EXCEPTION
  WHEN others THEN
    NULL;
END
...
DROP TABLE IF EXISTS t30984;
CREATE TABLE t30984 (
  a INT PRIMARY KEY,
  b INT,
  c INT
);
CREATE INDEX t30984_b_idx ON t30984 (b) INCLUDE (c, a);
...
CREATE UNIQUE INDEX t30984_b_idx ON t30984 (b) INCLUDE (c, a);
...
CREATE UNIQUE INDEX t30984_b_idx ON t30984 (b) INCLUDE (c, d);

-- Test 13: statement (line 83)
CREATE UNIQUE INDEX a_idx ON t (a) INCLUDE (b);

-- Test 14: statement (line 91)
DROP TABLE IF EXISTS t14601;
CREATE TABLE t14601 (a TEXT, b BOOLEAN);
...
DROP TABLE IF EXISTS t14601a;
CREATE TABLE t14601a (a TEXT, b BOOLEAN, c INT);
