...
WITH foo AS (INSERT INTO y VALUES (1) RETURNING *)
...
WITH
  ...
  u AS (INSERT INTO s SELECT i FROM t ORDER BY i ON CONFLICT (i) DO UPDATE SET i = s.i - 2 RETURNING s.i)
SELECT * FROM r, u;
...
INSERT INTO t SELECT i + 4, j + 4 FROM u
ON CONFLICT (i) DO UPDATE SET j = EXCLUDED.j
RETURNING *;
...
INSERT INTO t SELECT sum(i), sum(j) FROM u ON CONFLICT (i) DO UPDATE SET j = 1 RETURNING *;
...
WITH v AS (SELECT * FROM u WHERE i = 0)
INSERT INTO u SELECT i, j + 1 FROM v
ON CONFLICT (i) DO UPDATE SET j = EXCLUDED.j
RETURNING *;
...
INSERT INTO u VALUES (0, 42), (1, 42)
ON CONFLICT (i) DO UPDATE SET j = 52
RETURNING *;

WITH v AS (SELECT i, j FROM u WHERE i IN (0, 1) ORDER BY i)
INSERT INTO u SELECT i, j + 1 FROM v
ON CONFLICT (i) DO UPDATE SET j = EXCLUDED.j + 99
RETURNING *;
...
WITH
  v AS (
    DELETE FROM u
    WHERE ctid IN (SELECT ctid FROM u ORDER BY i LIMIT 1)
    RETURNING *
  ),
  w AS (
    DELETE FROM u
    WHERE ctid IN (SELECT ctid FROM u ORDER BY i LIMIT 2)
    RETURNING *
  )
SELECT * FROM w;
...
CREATE INDEX ints_n_idx ON ints(n);
...
DROP TABLE IF EXISTS t100561a;
DROP TABLE IF EXISTS t100561b;
