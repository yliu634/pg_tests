...
-- Provide Cockroach-style floor-division operator used by these tests.
DROP OPERATOR IF EXISTS //(bigint, smallint);
DROP FUNCTION IF EXISTS crdb_int_div(bigint, smallint);

CREATE FUNCTION crdb_int_div(lhs bigint, rhs smallint)
RETURNS bigint
LANGUAGE SQL
IMMUTABLE
STRICT
...
CREATE OPERATOR // (
  LEFTARG = bigint,
  RIGHTARG = smallint,
  PROCEDURE = crdb_int_div
);

DROP TABLE IF EXISTS many_types;
...
INSERT INTO many_types VALUES
  (
    ...
    '01:02:03'::time,
    '1 day'::interval,
    '[0, 1, {"a": ["foo", {"b": 3}]}, 3, 4, 1]'::jsonb,
    '\x01'::bytea,
    'foo',
    B'1011'::varbit,
    ARRAY['2', 'a'],
    '2001-01-18 01:00:00.001'::timestamp,
    true
  ),
  (
    '10.0.0.5'::inet,
    ...
    '12:34:56'::time,
    '2 hours'::interval,
    '[0, 1, {"a": ["foo", {"b": 3}]}, 3, 4, 0]'::jsonb,
    '\x0203'::bytea,
    'bar',
    B'11'::varbit,
    ARRAY['2', 'a'],
    '2001-01-18 02:00:00.001'::timestamp,
    false
  );
...
SELECT _json - (_int::int) FROM many_types;
...
SELECT _json -> (_int::int) -> 'a' = '["foo", {"b": 3}]' FROM many_types;
...
SELECT '[2, "hi", {"b": ["bar", {"c": 4}]}]'::jsonb -> (_int::int) FROM many_types;
...
-- Cockroach-only optimizer settings.
