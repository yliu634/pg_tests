...
SELECT (1::TEXT, 2::TEXT) > (1::TEXT, 'hi'::TEXT) FROM tb;

-- Test 15: statement (line 146)
SELECT (1, 2, 0) > (1, 2, 3) FROM tb;
...
SELECT ((1, 2::NUMERIC), 'equal') = ((1, 2.0), 'equal') AS a,
       ((1, 2::NUMERIC), 'equal') = ((1, 2.0), 'not equal') AS b
	   FROM tb;

-- Test 22: query (line 186)
SELECT ((1, 2::NUMERIC), 'equal') = ((1, 2.1), 'equal') AS a
  FROM tb;
...
SELECT ((1, 2::TEXT), 'equal') = ((1, 'huh'::TEXT), 'equal') FROM tb;
...
CREATE INDEX uvw_u_v_w_idx ON uvw (u, v, w);
...
-- query TT colnames
SELECT (1, 2, 'hello', NULL, NULL) AS r,
       (true, NULL, (false, 6.6, false)) AS s
  FROM tb;

-- Test 55: query (line 680)
SELECT (1, '2'::TEXT) FROM tb;

-- Test 56: query (line 686)
SELECT (1, '2'::TEXT, true) FROM tb;

-- Test 57: query (line 691)
SELECT (1, '2'::TEXT, true) FROM tb;

-- Test 58: query (line 696)
SELECT (1, 'asd'::TEXT, true) FROM tb;
...
SELECT (((1, 2), 'value')) = (((1, 2), 'value')) AS nnnn,
       (((1, 2), 'value')) = ((1, 2), 'value') AS nnnu,
       (((1, 2), 'value')) = ((1, 2), 'value') AS nnun,
       (((1, 2), 'value')) = ((1, 2), 'value') AS nnuu,
       ((1, 2), 'value') = (((1, 2), 'value')) AS nunn,
       ((1, 2), 'value') = ((1, 2), 'value') AS nunu,
       ((1, 2), 'value') = ((1, 2), 'value') AS nuun,
       ((1, 2), 'value') = ((1, 2), 'value') AS nuuu,
       ((1, 2), 'value') = (((1, 2), 'value')) AS unnn,
       ((1, 2), 'value') = ((1, 2), 'value') AS unnu,
       ((1, 2), 'value') = ((1, 2), 'value') AS unun,
       ((1, 2), 'value') = ((1, 2), 'value') AS unuu,
       ((1, 2), 'value') = (((1, 2), 'value')) AS uunn,
       ((1, 2), 'value') = ((1, 2), 'value') AS uunu,
       ((1, 2), 'value') = ((1, 2), 'value') AS uuun,
       ((1, 2), 'value') = ((1, 2), 'value') AS uuuu
 FROM tb;

-- Test 64: query (line 747)
SELECT (ROW(pow(1, 10.0) + 9), 'a' || 'b') = (ROW(sqrt(100.0)), 'ab') AS a,
       (ROW(pow(1, 10.0) + 9), 'a' || 'b') = (ROW(sqrt(100.0)), 'ab') AS b
 FROM tb;
...
SELECT ((1,2,3)).f1 FROM tb;

-- query ITBITB colnames
SELECT ((1,'2'::TEXT,true)).f1,
       ((1,'2'::TEXT,true)).f2,
       ((1,'2'::TEXT,true)).f3,
       (ROW(1,'2'::TEXT,true)).f1,
       (ROW(1,'2'::TEXT,true)).f2,
       (ROW(1,'2'::TEXT,true)).f3
 FROM tb;
...
SELECT (ROW(1,'2'::TEXT,true)).* FROM tb;

-- Test 71: query (line 854)
SELECT ((ROW(1,'2'::TEXT,true)).*, 456) FROM tb;
...
FROM (
  SELECT ROW(1,'2'::TEXT,true) AS x FROM tb
) AS s;
...
SELECT (x).f1, (x).f2
    FROM (SELECT ROW(a, b) AS x FROM (VALUES (1, 'one')) AS t(a, b)) AS s;
...
SELECT (x).f1, (x).f2
    FROM (SELECT ROW(a, b) AS x FROM t) AS s
ORDER BY 1
	   LIMIT 1;
...
SELECT ROW() IS NOT DISTINCT FROM ROW();
...
SELECT ((ARRAY[t58439.*])[1]).* FROM t58439;

-- Test 87: query (line 965)
SELECT ((ARRAY[t58439.*])[3]).* FROM t58439;

-- Test 88: query (line 976)
SELECT (1::INT, NULL::INT)
UNION
SELECT (1::INT, NULL::INT);

-- Test 89: statement (line 987)
CREATE TABLE t(x INT);
CREATE TABLE t40297 AS SELECT g FROM generate_series(NULL::INT, NULL::INT) AS g;

-- Test 90: query (line 991)
SELECT COALESCE(ROW(), NULL::record) FROM t40297;

-- Test 91: query (line 996)
SELECT COALESCE(ROW()::record, NULL::record) FROM t40297;

-- Test 92: statement (line 1001)
SELECT COALESCE(ROW(NULL::INT), ROW(1)) FROM t40297;

-- Test 93: statement (line 1005)
SELECT COALESCE(ROW(NULL::INT)::t, ROW(1)::t) FROM t40297;

-- Test 94: statement (line 1009)
SELECT COALESCE(ROW(NULL::INT)::t, NULL::t) FROM t40297;

-- Test 95: query (line 1013)
SELECT COALESCE((SELECT '(1)'::t), NULL::t) FROM t40297;

-- Test 96: statement (line 1018)
SELECT COALESCE(ROW(1)::t, NULL::t) FROM t40297;
...
SELECT
  col
FROM
  (VALUES (NULL), ((ARRAY[]::RECORD[])), ((ARRAY[ROW()])))
    AS t(col)
ORDER BY
  col ASC;
...
SELECT
  col
FROM
  (VALUES ((ARRAY[]::RECORD[])), ((ARRAY[ROW(1)])))
    AS t(col)
ORDER BY
  col ASC;
...
SELECT
  col
FROM
  (VALUES ((ARRAY[]::RECORD[])), ((ARRAY[(1,'2'::TEXT)])), ((ARRAY[(1,'cat'::TEXT)])))
    AS t(col)
ORDER BY
  col ASC;
...
SELECT CASE WHEN b THEN NULL ELSE 1 END AS a FROM t78159;

-- Test 118: query (line 1204)
SELECT CASE WHEN b THEN 1 ELSE NULL END AS a FROM t78159;

-- Test 119: statement (line 1213)
SELECT CASE WHEN false THEN 1 ELSE 2 END AS a;

-- Test 120: statement (line 1218)
SELECT CASE WHEN false THEN 1 ELSE NULL END AS a;

-- Test 121: statement (line 1221)
SELECT CASE WHEN false THEN NULL ELSE 2 END AS b;
...
-- query T
SELECT CASE
         WHEN a = 1 THEN NULL::TEXT
         WHEN a = 2 THEN ROW(NULL, NULL, 1.1e3::FLOAT)::TEXT
         WHEN a = 3 THEN ROW(NULL, 1.1::DECIMAL, NULL)::TEXT
         WHEN a = 4 THEN ROW(1::INT, NULL, NULL)::TEXT
         WHEN a = 5 THEN NULL::TEXT
         ELSE NULL::TEXT
       END AS r
FROM t109105
ORDER BY r;
