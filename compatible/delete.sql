...
DROP VIEW IF EXISTS kview CASCADE;
DROP TABLE IF EXISTS kv, unindexed, indexed, t29494, t33361, family, a, u_a, u_b, u_c, u_d, pindex, t99630a, t99630b, t107634, t108166, t CASCADE;
...
CREATE UNIQUE INDEX kv_foo ON kv (v);
CREATE INDEX kv_bar ON kv (k, v);
...
-- This test expects an error (column nonexistent does not exist)
...
-- PostgreSQL doesn't support ORDER BY with DELETE
...
CREATE TABLE indexed (id int primary key, value int, other int);
...
-- PostgreSQL doesn't support LIMIT with DELETE without ORDER BY
-- SELECT count(*) FROM (DELETE FROM unindexed LIMIT 2 RETURNING v) AS tmp;

-- Test 45: query (line 219)
-- PostgreSQL doesn't support LIMIT with DELETE without ORDER BY
-- SELECT count(*) FROM (DELETE FROM unindexed LIMIT 1 RETURNING v) AS tmp;

-- Test 46: query (line 224)
-- PostgreSQL doesn't support LIMIT with DELETE without ORDER BY
-- SELECT count(*) FROM (DELETE FROM unindexed LIMIT 5 RETURNING v) AS tmp;
...
CREATE TABLE t33361(x INT PRIMARY KEY, y INT UNIQUE, z INT);
INSERT INTO t33361 VALUES (1, 2, 3);
...
-- This test expects an error (column y was dropped)
...
BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;
ALTER TABLE family ADD COLUMN z INT;
...
CREATE TABLE u_a (a INT, b TEXT, c INT);
CREATE TABLE u_b (a INT, b TEXT);
CREATE TABLE u_c (a INT, b TEXT, c INT);
CREATE TABLE u_d (
  a INT,
  b INT
);
...
-- ALTER TABLE u_a SET (schema_locked=false) -- CockroachDB-specific

-- Test 90: statement (line 464)
TRUNCATE u_a;

-- Test 91: statement (line 467)
-- ALTER TABLE u_a RESET (schema_locked) -- CockroachDB-specific
...
-- PostgreSQL doesn't support ORDER BY with DELETE...USING
...
CREATE INDEX ON pindex (a) WHERE a > 3;
...
SELECT a FROM pindex WHERE a > 3;
...
CREATE TABLE t99630a (a INT);
CREATE INDEX t99630a_idx ON t99630a (a) WHERE a > 0;
...
SELECT a FROM t99630a WHERE a > 0;

-- Test 112: statement (line 597)
CREATE TABLE t107634 (a INT);

-- Test 113: statement (line 600)
-- CockroachDB allows ORDER BY with aggregates in DELETE, PostgreSQL does not
