...
DROP TABLE IF EXISTS abc_decimal;
DROP TABLE IF EXISTS def_decimal;
...
-- Test 3: query (line 9)
SELECT * from abc WHERE EXISTS (SELECT 1 FROM def WHERE a=d);

-- Test 4: query (line 17)
SELECT * from abc WHERE EXISTS (SELECT 1 FROM def WHERE a=f);

-- Test 5: query (line 23)
SELECT * from abc WHERE EXISTS (SELECT 1 FROM def WHERE a=d AND c=e);

-- Test 6: query (line 30)
SELECT a, b, c FROM abc WHERE EXISTS (SELECT 1 FROM def WHERE a=d OR a=e);

-- Test 7: query (line 38)
SELECT c FROM abc WHERE EXISTS (SELECT 1 FROM def WHERE a=d OR a=e);

-- Test 8: query (line 46)
SELECT a, b, c FROM abc WHERE NOT EXISTS (SELECT 1 FROM def WHERE a=d OR a=e);

-- Test 9: query (line 51)
SELECT c FROM abc WHERE NOT EXISTS (SELECT 1 FROM def WHERE a=d OR a=e);

-- Test 10: statement (line 60)
-- CockroachDB schema_locked setting; not supported in PostgreSQL.
...
DROP TABLE abc;
DROP TABLE def;
DROP TABLE abc_decimal;
DROP TABLE def_decimal;
